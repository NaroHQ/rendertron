...
test.failing("renders shadow DOM - polyfill loader", async (t) => {
  const res = await server.get(
...
test.failing(
  "renders shadow DOM - polyfill loader - different flag",
  async (t) => {
    const res = await server.get(
...
test.failing(
  "script tags and link[rel=import] tags are stripped",
  async (t) => {
